(function (c) {
    var e = function (a, b, d, f) { this.target = a; this.url = b; this.html = []; this.effectQueue = []; this.options = c.extend({ ssl: !1, limit: null, key: null, layoutTemplate: "<ul>{entries}</ul>", entryTemplate: '<li><a href="{url}">[{author}@{date}] {title}</a><br/>{shortBodyPlain}</li>', tokens: {}, outputMode: "json", effect: "show", error: function () { console.log("jQuery RSS: url doesn't link to RSS-Feed") }, success: function () { } }, d || {}); this.callback = f || this.options.success }; e.htmlTags = "doctype,html,head,title,base,link,meta,style,script,noscript,body,article,nav,aside,section,header,footer,h1-h6,hgroup,address,p,hr,pre,blockquote,ol,ul,li,dl,dt,dd,figure,figcaption,div,table,caption,thead,tbody,tfoot,tr,th,td,col,colgroup,form,fieldset,legend,label,input,button,select,datalist,optgroup,option,textarea,keygen,output,progress,meter,details,summary,command,menu,del,ins,img,iframe,embed,object,param,video,audio,source,canvas,track,map,area,a,em,strong,i,b,u,s,small,abbr,q,cite,dfn,sub,sup,time,code,kbd,samp,var,mark,bdi,bdo,ruby,rt,rp,span,br,wbr".split(",");
    e.prototype.load = function (a) { var b = "http" + (this.options.ssl ? "s" : "") + "://ajax.googleapis.com/ajax/services/feed/load?v=1.0&output=" + this.options.outputMode + "&callback=?&q=" + encodeURIComponent(this.url); null != this.options.limit && (b += "&num=" + this.options.limit); null != this.options.key && (b += "&key=" + this.options.key); c.getJSON(b, a) }; e.prototype.render = function () {
        var a = this; this.load(function (b) {
            try { a.feed = b.responseData.feed, a.entries = b.responseData.feed.entries } catch (d) {
                return a.entries = [], a.feed = null,
                a.options.error.call(a)
            } b = a.generateHTMLForEntries(); a.target.append(b.layout); 0 !== b.entries.length && a.appendEntriesAndApplyEffects(c("entries", b.layout), b.entries); 0 < a.effectQueue.length ? a.executeEffectQueue(a.callback) : c.isFunction(a.callback) && a.callback.call(a)
        })
    }; e.prototype.appendEntriesAndApplyEffects = function (a, b) { var d = this; c.each(b, function (b, e) { var c = d.wrapContent(e); "show" === d.options.effect ? a.before(c) : (c.css({ display: "none" }), a.before(c), d.applyEffect(c, d.options.effect)) }); a.remove() };
    ...
    }; e.prototype.getValueForToken = function (a, b) { var d = this.getTokenMap(b), c = a.replace(/[\{\}]/g, ""), c = d[c]; if ("undefined" != typeof c) return "function" == typeof c ? c(b, d) : c; throw Error("Unknown token: " + a); }; c.fn.rss = function (a, b, c) { (new e(this, a, b, c)).render(); return this }
})(jQuery);